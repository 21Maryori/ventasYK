<?php
	...
	class mainModel{
	    ...
	    public function obtenerProductosConStockBajo() {
			...
			$query = $conexion->prepare("SELECT producto_nombre AS nombre, producto_stock_total AS stock FROM producto WHERE producto_stock_total <= :stock_minimo");
			$stock_minimo = 5;  // Definir el umbral de stock bajo
			$query->bindParam(":stock_minimo", $stock_minimo, PDO::PARAM_INT);
			$query->execute();
